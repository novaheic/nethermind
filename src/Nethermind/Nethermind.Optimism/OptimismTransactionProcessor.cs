    ...
    ) : TransactionProcessorBase(blobBaseFeeCalculator, specProvider, worldState, virtualMachine, codeInfoRepository, logManager)
{
    ...
    protected override TransactionResult Execute(Transaction tx, ITxTracer tracer, ExecutionOptions opts)
    {
        ...
        if (!result && tx.IsDeposit() && result.Error != TransactionResult.ErrorType.BlockGasLimitExceeded)
        {
            ...
            tracer.MarkAsFailed(tx.To!, txGasLimit, [], $"failed deposit: {result.ErrorDescription}");
        ...
        in UInt256 effectiveGasPrice, out UInt256 premiumPerGas, out UInt256 senderReservedGasPayment, out UInt256 blobBaseFee)
    {
        ...
        if (validate && !tx.IsDeposit())
        {
            ...
            long txGasLimit = ToSignedGas(tx.GasLimit, "transaction gas limit");
            UInt256 maxOperatorCost = costHelper.ComputeOperatorCost(txGasLimit, header, WorldState);
        ...
        in TransactionSubstate substate, long spentGas, in UInt256 premiumPerGas, in UInt256 blobGasFee, int statusCode)
    {
        if (!tx.IsDeposit())
        {
            ...
            {
                UInt256 operatorCostMax = costHelper.ComputeOperatorCost(ToSignedGas(tx.GasLimit, "transaction gas limit"), header, WorldState);
        ...
        in TransactionSubstate substate, in long unspentGas, in UInt256 gasPrice, int codeInsertRefunds, long floorGas)
    {
        ...
        if (tx.IsDeposit() && !opSpecHelper.IsRegolith(header))
        {
            ...
            long txGasLimit = ToSignedGas(tx.GasLimit, "transaction gas limit");
            long gas = tx.IsOPSystemTransaction ? 0 : txGasLimit;
            return gas;
        }
